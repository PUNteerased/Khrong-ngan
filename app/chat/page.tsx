...
function ChatPageInner() {
  ...
  /** โหลดประวัติเมื่อเปิดจากลิงก์ /chat?sessionId=... */
  useEffect(() => {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    if (!getStoredToken()) {
      toast.error("กรุณาเข้าสู่ระบบก่อนแชท")
      return
    }
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-card">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-semibold text-foreground">LaneYa AI</h1>
              <p className="text-xs text-success">ออนไลน์</p>
            </div>
          </div>
        </div>
        <Link href="/history" title="ประวัติแชท">
          <Button variant="ghost" size="icon" className="h-8 w-8">
